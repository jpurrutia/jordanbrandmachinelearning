...
##### DATA MANIPULATION #####
#########################################################
cron_data <- Jordan[,-c(1,2,12,13,14,15,16,17,20,21,22,23,24,25,26)]
View(cron_data)
cron_data %>% correlate()
...
##### AVG INTER-ITEM CORR #####
#########################################################
inter_item <- cron_data %>% correlate() %>% dplyr::select(-rowname) %>% colMeans(na.rm = TRUE)
inter_item
mean(inter_item)
